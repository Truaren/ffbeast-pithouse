...
export const AutoProfiles = () => {
  const { preferences, addAutoProfile, removeAutoProfile } = useAppPreferencesStore();
      ...
      <div className="autoprofiles-container">
        <div className="autoprofiles-body">
          <p className="description" style={{ marginBottom: "1rem" }}>
            Specify an exact executable name (e.g., <strong>acs.exe</strong>, <strong>forza_horizon5.exe</strong>) and bind a profile to it. When the application detects that process, it will automatically switch your wheel to the mapped profile.
          </p>
            ...
            <div className="auto-profile-form">
              <div style={{ display: 'flex', gap: '0.5rem', flex: 1 }}>
                <input 
                  ...
                  style={{ flex: 1 }}
                />
                <Button variant="secondary" onClick={() => void handleBrowseExe()} style={{ padding: '0 0.75rem' }}>
                  <FolderOpen size={18} />
                </Button>
              </div>
              <select 
              ...
              <Button 
                ...
                disabled={!newAutoExe || !newAutoProfileId || profiles.length === 0}
              >
                Add Rule
              </Button>
            </div>
          ...
          <div className="platform-settings" style={{ marginTop: "2rem" }}>
            ...
            <div className="auto-profile-list">
              {(preferences.autoProfiles || []).length === 0 ? (
                <div className="empty-message">No automatic profile rules configured yet.</div>
              ) : (
                (preferences.autoProfiles || []).map((mapping, idx) => {
                  const boundProfile = profiles.find(p => p.id === mapping.profileId);
                  return (
                    <div key={idx} className="setting-item auto-profile-item">
                      <div className="setting-info">
                        <span className="setting-label">{mapping.exeName}</span>
                        <span className="setting-description">Bound to: {boundProfile ? boundProfile.name : "Unknown Profile"}</span>
                      </div>
                      <button 
