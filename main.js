...
// IPC: register a global shortcut for wheel recentering
ipcMain.handle("set-recenter-shortcut", (_event, shortcut) => {
  if (recenterShortcut) {
    globalShortcut.unregister(recenterShortcut);
  }
  ...
  if (shortcut) {
    try {
      const success = globalShortcut.register(shortcut, () => {
        if (mainWindow) {
          mainWindow.webContents.send("recenter-wheel");
        }
      });
      if (success) {
        recenterShortcut = shortcut;
...
ipcMain.handle("pedals:getReadme", () => {
  try {
    ...
    console.error("Failed to read pedals README:", err);
  }
  return null;
});
